...
## 5. 이미 동기화된 글을 수정했을 때 (재동기화)

자동 스케줄은 "어제 작성된 글"만 매일 한 번 가져오기 때문에, 이미 지나간 날짜의 글을 나중에 다시 고치면 자동으로는 반영되지 않습니다.

**가장 쉬운 방법**: Actions 탭 → `TIL Notion Sync` → **Run workflow** → `target_date` 칸은 비워두고 그냥 실행 버튼만 누르세요. 이 경우 Notion 데이터베이스 전체를 다시 훑어서, 수정된 내용까지 전부 최신 상태로 덮어씁니다. 매번 날짜를 입력할 필요 없습니다.

(글이 아주 많아지면 전체 재동기화가 느려질 수 있는데, 그럴 땐 `target_date`에 날짜(`2026-07-14`)를 넣어 그 날짜 하나만 재동기화할 수도 있습니다.)

> ⚠️ 주의: 재동기화 시 **Notion 페이지 제목을 바꾸지 않았다면** 같은 파일이 그대로 덮어써집니다. 제목을 바꿨다면 파일명(슬러그)도 같이 바뀌어서 예전 파일이 남아있을 수 있어요 — 이 경우 `TIL/{연도}/{월}/` 폴더에서 오래된 파일을 수동으로 지워주세요.
